...
namespace DWBC
{
    ...
    void RobotData::CalcTaskTorque(bool hqp, bool init)
    {
        ...
        if (hqp)
        {
            ...
            for (int i = 0; i < ts_.size(); i++)
            {
                if (i == 0)
                {
                    CalcTaskTorqueQP(ts_[i], MatrixXd::Identity(model_dof_, model_dof_), torque_limit, torque_grav_, NwJw, J_C_INV_T, P_C, init);

                    torque_task_ = ts_[i].J_kt_ * ts_[i].Lambda_task_ * (ts_[i].f_star_ + ts_[i].f_star_qp_);
                }
                else
                {
                    CalcTaskTorqueQP(ts_[i], ts_[i - 1].Null_task_, torque_limit, torque_grav_, NwJw, J_C_INV_T, P_C, init);
    ...
    */
    ...
    void RobotData::CalcTaskTorqueQP(TaskSpace &ts_, const MatrixXd &task_null_matrix_, const VectorXd &torque_limit, const VectorXd &torque_prev, const MatrixXd &NwJw, const MatrixXd &J_C_INV_T, const MatrixXd &P_C, bool init_trigger)
    {
        ...
        for (int i = 0; i < cc_.size(); i++)
        {
            ...
            A_const_a.block(constraint_idx, contact_idx, 4, 6) = cc_[i].GetZMPConstMatrix4x6();
            A_const_a.block(constraint_idx + CONTACT_CONSTRAINT_ZMP, contact_idx, 6, 6) = cc_[i].GetForceConstMatrix6x6();

            constraint_idx += cc_[i].constraint_number_;
